...
/** Geo helpers shared by the anti-cheat checks and the distance labels in the app. */
object Geo {

    private const val EARTH_RADIUS_M = 6_371_000.0

    /** How close the phone must be to the pin for a GPS check-in to count. */
    const val MAX_CHECKIN_DISTANCE_M = 150.0

    /**
     * Anything faster than this between two consecutive check-ins means the user
     * either teleported or is faking GPS, so the check-in gets flagged for a moderator.
     */
    ...
    /** Latitude/longitude deltas covering [radiusMeters], used to pre-filter rows in SQL. */
